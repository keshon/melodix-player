...
type EncodeOptions struct {
	Volume                  float32          // change audio volume (1.0=normal)
...
func (opts *EncodeOptions) Validate() error {
	if opts.Volume < 0 || opts.Volume > 1.0 {
		return errors.New("out of bounds volume (0.0-1.0)")
	}
...
func (e *EncodeSession) run() {
	...
	// slog.Info("Application", e.options.Application)
	// slog.Info("AudioFilter", e.options.AudioFilter)
	// slog.Info("Bitrate", e.options.Bitrate)
	// slog.Info("BufferedFrames", e.options.BufferedFrames)
	// slog.Info("Channels", e.options.Channels)
	// slog.Info("CompressionLevel", e.options.CompressionLevel)
	// slog.Info("EncodingLineLog", e.options.EncodingLineLog)
	// slog.Info("FfmpegBinaryPath", e.options.FfmpegBinaryPath)
	// slog.Info("FrameDuration", e.options.FrameDuration)
	// slog.Info("FrameRate", e.options.FrameRate)
	// slog.Info("PacketLoss", e.options.PacketLoss)
	// slog.Info("RawOutput", e.options.RawOutput)
	// slog.Info("ReconnectAtEOF", e.options.ReconnectAtEOF)
	// slog.Info("ReconnectDelayMax", e.options.ReconnectDelayMax)
	// slog.Info("ReconnectOnHttpError", e.options.ReconnectOnHttpError)
	// slog.Info("ReconnectOnNetworkError", e.options.ReconnectOnNetworkError)
	// slog.Info("ReconnectStreamed", e.options.ReconnectStreamed)
	// slog.Info("StartTime", e.options.StartTime)
	// slog.Info("Threads", e.options.Threads)
	// slog.Info("UserAgent", e.options.UserAgent)
	// slog.Info("VBR", e.options.VBR)
	// slog.Info("Volume", e.options.Volume)

	// Launch ffmpeg with a variety of different fruits and goodies mixed togheter
	args := []string{
		"-stats", // not need to specify, on by default
		"-i", inFile,
		"-vn",
		"-map", "0:a",
		"-acodec", "libopus",
	...
	filters := []string{
		fmt.Sprintf("volume=%v", e.options.Volume),
	}
	...
	if _, err := os.Stat(ffmpegPath); errors.Is(err, os.ErrNotExist) {
		ffmpegPath = "" // reset path if it's not valid
	}
	ffmpeg := exec.Command(ffmpegPath+"ffmpeg", args...)

	slog.Info(ffmpeg.Args)
...
func (e *EncodeSession) writeMetadataFrame() {
	...
	if e.pipeReader == nil {
		...
		err = ffprobe.Wait()
		if err != nil {
			slog.Error("FFprobe Error:", err)
			return
		}
		var ffprobeData *FFprobeMetadata
		err = json.Unmarshal(cmdBuf.Bytes(), &ffprobeData)
		if err != nil {
			slog.Error("Erorr unmarshaling the FFprobe JSON:", err)
			return
		}
		...
		if err != nil {
			slog.Error("Could not convert bitrate to int:", err)
			return
		}
	...
	if err != nil {
		slog.Error("Couldn't write json len:", err)
		return
	}
...
func (e *EncodeSession) handleStderrLine(line string) {
	...
	if err != nil {
		slog.Error("Error parsing ffmpeg stats:", err)
	}
	...
	for {
		...
		if err != nil {
			if !errors.Is(err, io.EOF) && !errors.Is(err, io.ErrUnexpectedEOF) {
				slog.Error("Error reading ffmpeg stdout:", err)
			}
			break
		}
		...
		if err != nil {
			slog.Error("Error writing opus frame:", err)
			break
		}
	}
}
